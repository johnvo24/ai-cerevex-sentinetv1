...
def save_best_checkpoint_to_gdrive(model_dir):
  ...
  GDrive().upload_file(
    ...
    file_path=os.path.join("model", f"{model_dir}/best_checkpoint.tar"),
...
def save_checkpoint(model_dir, epoch, model, optimizer, is_the_best=False):
  makedir("model", model_dir) # Create model_dir
  if is_the_best: # Save with name best_checkpoint
    file_path = os.path.join("model", f"{model_dir}/best_checkpoint.tar")
  else: # Save with name checkpoint_epoch
    file_path = os.path.join("model", f"{model_dir}/checkpoint_{epoch}.tar")
...
def load_checkpoint(model_dir, model, optimizer, epoch=None, is_the_best=False, map_location=device()):
  ...
  if is_the_best:
    ...
    if epoch == None:
      cp_files = [f for f in os.listdir(os.path.join("model", model_dir)) if f.startswith("checkpoint_") and f.endswith(".tar")]
      latest_epoch = 0
